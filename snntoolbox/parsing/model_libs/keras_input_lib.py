...
def load(path, filename, **kwargs):
    ...
    model.summary()
    # run the model once for later code to work (to get output shape)
    x = model(np.random.random((10, 1)))
    print("called loaded model with result:", x)
    # print("in/out shape", model.in_shape, model.out_shape)
